...
using all 480 acquisitions.  The temporal-stability analysis used
...
Probe & Butterfly iQ+ matrix-array probe with custom firmware \\
...
\begin{figure*}[t]
  ...
  \caption{Ablation of the Dower Coppler components on the same
    ...
    coherence-strength image, and $R^2$ alone is the unsigned
    phase-linearity quality map.  The final Dower Coppler panel uses
    $v_\phi G_R R^2$.  Images are cropped to the lateral range
    $-1.5$ to $1.5$~cm.}
...
\begin{figure*}[t]
  ...
  \caption{Comparison of conventional lag-1 Kasai color Doppler and
    ...
    \textbf{Top left}: Kasai velocity, computed from $\angle R_1$.
    \textbf{Bottom left}: multi-lag phase velocity $v_\phi$ from the
    weighted phase-slope fit.  The two velocity maps are stacked
    vertically and shown in mm/s with the same symmetric color scale.
    \textbf{Right}:
...
\begin{figure*}[t]
  ...
  \caption{Regions used for the CNR and gCNR measurements in
    \cref{fig:cnr}.  Cyan circles indicate the tolerant circular signal
    ROIs used for measurement; each circle is centered inside the
    exported vessel mask and has at least 80\% pixel overlap with that
    mask.  The dashed green rectangle is the shared background ROI used
    for every vessel and every Doppler display.}
  \label{fig:cnr_rois}
\end{figure*}
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Left}: CNR in dB across six vessel ROIs.  Dower
    ...
    Kasai color Doppler CNR is reported in the text but omitted from
    this panel.
...
two non-overlapping halves (0--239 and 240--479), recomputed median
...
background, the Dower Coppler sign agreed for 72.4\% of 31,810 finite
nonzero pixels.  Restricting the comparison to the highest-magnitude
pixels gave substantially higher agreement: 92.2\% for the top 10\%
of pixels and 94.5\% for the top 5\%.  In the tolerant circular
vessel ROIs used for the CNR analysis, the sign agreed between halves
for 98.8\% of finite nonzero pixels (250 total ROI pixels).
...
\begin{figure*}[t]
  ...
  \caption{Split-half consistency of Dower Coppler on the fine-elevation
    plane at $y=-0.44$~mm.  \textbf{Left}: median Dower Coppler map from
    acquisitions 0--239.  \textbf{Middle}: median map from acquisitions
    240--479.  Both Dower panels use one shared symmetric
    97th-percentile color scale.  \textbf{Right}: sign agreement within
    the top 10\% of finite nonzero pixels by maximum absolute Dower
    magnitude across the two halves; gray pixels are outside this
    high-magnitude subset.}
...
\begin{figure*}[t]
  ...
  \caption{Dower Coppler maps computed from different numbers of
    ...
    panels use one shared symmetric 97th-percentile color scale.  The
    vessel pattern and flow direction assignment remain stable from the full
    480-acquisition ultratrace down to 10 acquisitions.  Images are
    cropped to the lateral range $-1.5$ to $1.5$~cm.}
  \label{fig:stability}
\end{figure*}

The vessel pattern and flow direction assignments are stable from the
full 480-acquisition ultratrace down to as few as 10 acquisitions,
with background noise increasing as fewer acquisitions are included.
...
$y=-4$ to $4$~mm.  This figure is the main 3D matrix-imaging result:
the directional vascular pattern is recovered across adjacent
elevation planes rather than appearing only in one selected 2D slice.
...
\begin{figure*}[t]
  ...
  \caption{Dower Coppler maps across elevation planes 2--7 from the
    ...
    $4$~mm with 10 reconstructed elevation planes.  The panels use all
    480 acquisitions and one shared symmetric color scale computed from
    the 97th percentile of absolute Dower Coppler values across the
    displayed planes.  Images are cropped to the lateral range $-1.5$
    to $1.5$~cm.  The directional vascular pattern appears across the
    imaging volume, with similar vessel segments visible in adjacent
    planes.}
...
\begin{figure}[t]
  ...
  \caption{Dower Coppler image from a different recording: May 18,
    ...
    pulse PRF 1476.3~Hz (compounded-frame cadence 295.3~Hz),
    $f_0=2.75$~MHz, $c=1600$~m/s, SVD cutoff 0.08, and $K=5$.
    The main September 21 dataset uses a separate Head
    ultratrace, all 480 acquisitions for the CNR/fine-grid analysis,
    elevation planes 2--7 from the $y=-4$ to $4$~mm fine-elevation
    run for the volume montage, pulse PRF 1240.9~Hz
    (compounded-frame cadence 248.2~Hz), and the same $f_0$, $c$, SVD
    cutoff, and $K$.}
