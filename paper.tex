...
multiply-and-sum (TMAS), showing that multiplicative combinations of
slow-time information can improve power Doppler signal-to-noise ratio.
...
\begin{equation}
  ...
  \label{eq:tmas}
\end{equation}
This construction is motivated by TMAS~\cite{shen2022tmas}.
...
\begin{equation}
  ...
  \frac{\mathrm{PRF}}{2\pi}\,
  \frac{c}{2 f_0},
  \label{eq:phase_velocity}
\end{equation}
where $\mathrm{PRF}$ is the empirical pulse-repetition frequency, $c$
is the assumed speed of sound, and $f_0$ is the transmit center
frequency.  This is the standard axial Doppler conversion; no
beam-flow angle correction is applied.  The final image is
\begin{equation}
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Left}: CNR in dB across six vessel ROIs.  Dower
    Coppler achieves 11.2--40.1~dB, exceeding both power Doppler and
    Kasai color Doppler in all ROIs.
    \textbf{Right}: generalized CNR (1~$-$~histogram overlap).  All
    methods show high gCNR for the selected vessel/background
    separation, with Dower Coppler and power Doppler close to~1.0
    across all selections.}
  \label{fig:cnr}
\end{figure*}

Dower Coppler achieves 11.2--40.1~dB CNR across the six largest
inscribed circular vessel selections.  Power Doppler achieves
9.9--19.3~dB and Kasai color Doppler achieves -32.7--3.9~dB when the
signed Doppler maps are measured by magnitude, as in the viewer CNR
calculation.  The gCNR
metric confirms that the selected vessels are well separated from the
shared background region for all three displays, with Dower Coppler
and power Doppler at approximately 1.0 across all selections.

\Cref{fig:stability} demonstrates the temporal stability of the Dower
Coppler map by varying the number of Doppler buffers averaged.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig_temporal_stability.png}
  \caption{Dower Coppler maps computed from different numbers of Doppler
    buffers (same color scale throughout).  The vessel pattern and flow
    direction assignment are stable from 250 buffers down to 5~buffers.
    At a single buffer, the major vessels remain identifiable but
    background noise increases.}
  \label{fig:stability}
\end{figure*}

The vessel pattern and flow direction assignments are stable from 250
buffers down to as few as 5~buffers.  At a single buffer, the major
vessels remain identifiable but background noise increases
substantially.

\Cref{fig:all_planes} shows the Dower Coppler map across the middle
eight elevation planes from the recomputed H5 dataset, providing a qualitative check that the directional
...
\begin{figure*}[t]
  ...
  \caption{Dower Coppler maps across the middle eight elevation planes
    from the recomputed H5 dataset using acquisitions 200--399 (same
    color scale).  The directional vascular
...
\begin{figure}[t]
  ...
  \caption{Dower Coppler image from a different recording: May 18,
    ...
    the middle eight elevation planes for the volume montage, PRF
    1244.2~Hz, and the same $f_0$, $c$, SVD cutoff, and $K$.}
...
strength rather than pure velocity~\cite{rubin1994powerdoppler}.  Shen
and Guo's TMAS work is especially relevant because it uses products of
temporal autocorrelation terms to improve ultrafast power Doppler
SNR~\cite{shen2022tmas}; $G_R$ can be interpreted as a bounded
geometric-mean version of this product-of-lag-magnitudes idea.
...
Code and reproducible analysis scripts are available in the
\href{https://github.com/alegria-neurotech/caterpillar}{project repository}.
